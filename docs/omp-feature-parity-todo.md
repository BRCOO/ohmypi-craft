...
- [ ] 把 `FileAttachment.type === 'image'` 转成 OMP `ImageContent[]`。
- [ ] 传递 base64、MIME 类型和必要的尺寸限制。
- [ ] 对没有 base64 但有本地路径的图片执行安全读取或明确降级到 `inspect_image`。
- [ ] 区分 PDF、Office、音频和未知附件的处理策略。
- [ ] 传递 `streamingBehavior`。
- [ ] 传递单次 `thinkingOverride`，或在发送前临时调用 `set_thinking_level` 并在结束后恢复。
- [ ] 增加视觉模型、纯文本模型、超大图片和损坏附件测试。
...
| # | RPC 命令 | 当前状态 | Todo |
|---:|---|---|---|
| 1 | `prompt` | 部分接入 | 补图片、`streamingBehavior`、`prompt_result`、`agentInvoked:false` 和完整响应关联。 |
| 2 | `steer` | 已接入 | 增加图片 steer、失败反馈和队列状态显示。 |
| 3 | `follow_up` | 未接入 | 增加“当前轮结束后发送”入口和待发送队列 UI。 |
...
| 18 | `set_thinking_level` | 未接入 | 当前 Craft UI 只修改 BaseAgent 本地字段，必须真正发送给 OMP。 |
| 19 | `cycle_thinking_level` | 未接入 | 接入快捷切换，并显示 OMP 返回的实际 effort。 |
| 20 | `set_steering_mode` | 未接入 | 增加 `all/one-at-a-time` 设置与队列说明。 |
| 21 | `set_follow_up_mode` | 未接入 | 增加 `all/one-at-a-time` 设置。 |
| 22 | `set_interrupt_mode` | 未接入 | 增加 `immediate/wait` 设置并与停止按钮语义统一。 |
...
- [ ] `message_update`：区分文本 delta、thinking delta 和其他内容块。
...
- [ ] 输入 `/` 时展示名称、别名、描述、参数提示、子命令和来源。
- [ ] 支持来源标签：builtin、skill、extension、custom、mcp_prompt、file。
- [ ] 处理 `available_commands_update` 热更新。
- [ ] 命令输出不创建虚假的 assistant 消息。
...
- [ ] `setThinkingLevel` 必须发送 `set_thinking_level`，不能只修改 Craft 本地字段。
- [ ] 消费 `thinking_level_changed`。
- [ ] 显示 configured、effective、resolved 三种状态。
- [ ] 模型切换后重新校验可用等级。
- [ ] 单轮 override 完成后恢复会话等级。
...
- [ ] 保存最近 stderr，但避免把普通诊断当聊天错误。
- [ ] 对未知帧、丢失响应和重复 ID 提供计数。
- [ ] 子进程退出时保存 exit code/signal 和最后活动命令。
- [ ] 支持安全重启 OMP 会话并恢复状态。
- [ ] 增加诊断导出，包含脱敏后的状态、版本、命令能力和事件计数。
- [ ] OMP connection 测试必须执行真实 ready/get_state/get_available_models 流程。
