...
| 6 | `new_session` | 已接入 | 已支持 `parentSession`；OMP 会话启动时自动创建。 |
...
| 12 | `set_subagent_subscription` | 已接入 | 启动时设置 `progress` 订阅；实时 `subagent_lifecycle`/`subagent_progress` 驱动状态表。 |
| 13 | `get_subagents` | 已接入 | 启动/手动刷新拉取快照；列表、状态、进度和恢复已接入子智能体面板。 |
| 14 | `get_subagent_messages` | 已接入 | 详情页支持按 cursor 增量加载 transcript、空状态、错误和重试。 |
...
| 30 | `export_html` | 已接入 | 桌面菜单动作、成功提示、打开文件和失败反馈已接入。 |
| 31 | `switch_session` | 已接入 | Craft 会话启动时会切回持久化的 OMP sessionFile；崩溃重启后恢复。 |
| 32 | `branch` | 已接入 | 桌面会话菜单支持选择候选用户消息并创建分支，成功后回填 composer。 |
| 33 | `get_branch_messages` | 已接入 | 桌面分支对话框展示候选消息列表并映射到 Craft 消息。 |
| 34 | `get_last_assistant_text` | 已接入 | 已用于 OMP 分支连续性 fallback。 |
| 35 | `set_session_name` | 已接入 | Craft 重命名会同步到 live OMP；OMP → Craft 的显式标题同步仍待产品决策。 |
| 36 | `handoff` | 已接入 | 桌面菜单动作、结果提示、打开文件和失败反馈已接入。 |
| 37 | `get_messages` | 恢复对账已接入 | 首条新提示前核对用户消息数量、最后角色和最后文本，异常会持久化并阻止继续写错会话。 |
| 38 | `get_login_providers` | 已接入 | 显示 OMP 原生认证供应商、可用性和登录状态；驱动 onboarding/settings 登录页。 |
| 39 | `login` | 已接入 | 接入 OAuth URL、launchUrl、进度、取消和失败恢复；onboarding/settings 登录后刷新模型。 |
...
- [x] `config_update`：已同步思考等级、队列策略、模型、自动维护和运行时开关。
- [x] `session_info_update`：同步标题和 sessionId，不覆盖用户显式修改的 Craft 标题。
- [x] `session_shutdown`：区分正常关闭、切换、错误和被外部终止。
- [x] `extension_error`：显示扩展来源、错误堆栈摘要和禁用/重试动作。
- [x] `stderr`：不要把所有 stderr 都当用户错误；区分诊断日志和致命错误。

### 5.2 Agent 会话事件

- [x] `message_start`：建立稳定的 OMP messageId/turnId 映射。
- [x] `message_update`：区分文本 delta、thinking delta 和其他内容块。（文本与 thinking 已区分；其他内容块仍待补齐。）
- [x] `message_end`：保存 stopReason、usage、provider metadata 和错误信息。
...
- [x] 增加子智能体消息增量读取和错误重试。
- [ ] 支持隔离 worktree、patch、branch、apply/merge 结果展示（超出 Batch 01 范围）。
- [x] 主会话恢复时调用 `get_subagents` 重建 UI。
...
- [x] `get_login_providers` 驱动 OMP 登录页。
- [x] OAuth 登录支持 `open_url`、launchUrl、进度和取消。
- [ ] 对需要 API key 交互而 RPC 不支持的供应商给出明确终端指引或扩展上游协议。
- [ ] 增加 logout 能力设计（上游 RPC 暂无 logout；需协议扩展或安全调用 CLI）。
- [x] 当前 OMP connection 的 `validateStoredConnection` 不能永远返回 success。
- [x] 当前 OMP connection 的 `testConnection` 不能永远返回 null。
- [x] 增加 OMP 命令路径、版本、认证库、配置目录和模型访问诊断。
...
- [x] 建立 Craft sessionId ↔ OMP sessionId ↔ OMP sessionFile 映射。
- [x] 保存映射到持久化元数据。
- [x] 打开已有 Craft 会话时恢复对应 OMP session，而不是新建空 OMP 会话。
- [x] 使用 `get_messages` 对账用户消息数量、最后角色和最后消息文本。
- [ ] 定义 Craft 消息与 OMP entryId/messageId 的映射。
- [x] 子进程崩溃后自动重连原会话。
- [x] 如果原 OMP sessionFile 不存在，记录 `missing-session-file` 并报错，而不是静默重开。
- [ ] 避免 Craft 和 OMP 双方同时压缩/改写历史导致分叉。

### 9.2 分支与树

- [x] `get_branch_messages` 获取可分支用户消息并展示在桌面分支对话框。
- [x] `branch` 返回后更新两边的 session 映射并回填 composer。
...
- [x] `handoff` 生成新上下文并跳转到新会话。
- [x] 显示 handoff savedPath 和失败恢复。
- [x] `export_html` 支持默认路径和打开文件。
- [ ] `set_session_name` 双向同步。（Craft → OMP 已接入；OMP → Craft 不覆盖显式 Craft 标题。）
...
- [x] 手动 compact。
- [x] auto compaction 开关。
- [ ] soft/remote/snapcompact 策略。
- [x] 显示触发原因：threshold、overflow、idle、incomplete。
- [x] 显示动作：context-full、handoff、shake、snapcompact。
- [x] 显示 skipped、aborted、willRetry 和 errorMessage。
- [x] 压缩完成后刷新 context usage 和会话统计。
- [ ] 压缩完成后在消息视图中显示 compaction 分界和摘要。

### 10.2 重试与模型回退

- [x] auto retry 开关。
- [x] 显示 attempt/maxAttempts/delay/error。
- [x] 取消等待中的 retry。
- [x] 显示 fallback from/to/role。
- [x] 成功回退后同步当前模型。
- [ ] 区分 provider 错误、速率限制、上下文溢出和空响应。

### 10.3 上下文与统计

- [x] 消费 `get_state.contextUsage`。
- [x] 接入 `get_session_stats`。
- [x] 显示输入、输出、推理、缓存读写、总 token、费用和上下文百分比。
...
- [x] 定义 OMP TodoPhase/TodoItem 与 Craft 状态系统的关系，不能直接混为同一种概念。
- [x] 从 `get_state.todoPhases` 恢复 Todo。
- [x] 用 `set_todos` 保存桌面编辑。
- [x] 消费 reminder 和 auto-clear 事件。
- [x] 支持导入、导出、append、start、done、drop。
- [x] 子智能体 Todo 与主会话 Todo 分开显示。
...
- [x] 决定统一由 OMP 管理 MCP，还是由 Craft 管理后通过 Host Tool/URI 暴露：Craft Sources 为用户可见数据源事实源；OMP MCP 保持独立，Craft 不复刻。
- [x] 避免同一个 MCP server 在 Craft 和 OMP 中重复启动。
- [x] 把 Craft source tools 注册为 OMP host tools（通过 `craft-workspace://current/sources` 脱敏快照）。
- [x] 把 Craft 数据对象映射为稳定 URI（`craft-session://`、`craft-workspace://`）。
- [x] Host URI artifact 写入走 Craft 权限（safe/ask/allow-all）和审计，越权写入默认拒绝。
- [x] Host Tool 调用通过 `runPreToolUseChecks` 复用 Craft 权限决策。
- [ ] MCP OAuth、资源、提示、通知和重连状态进入桌面 UI（需上游 OMP RPC 暴露对应状态）。
- [ ] 支持 OMP tool discovery/BM25 激活状态（需上游 OMP RPC 暴露）。
...
- [x] Settings > AI 增加 “Oh My Pi subsystems” 状态面板，显示 Browser/LSP/GitHub/SSH/MCP 当前不被 OMP RPC 报告的状态。
- [ ] Browser：决定 OMP 自带浏览器和 Craft browser panel 的唯一控制面（需上游 OMP RPC 暴露 browser 状态）。
...
- [x] Host Tool 通过 `runPreToolUseChecks` 复用 Craft safe/ask/allow-all 权限决策。
- [x] Host URI artifact 写入通过同一套权限决策并记录审计。
- [x] Extension UI confirm 显示工具名、风险、参数和作用域。
- [ ] 明确 OMP approval mode 与 Craft safe/ask/allow-all 的映射，并在启动 OMP 时设置对应配置（需上游 OMP RPC 支持）。
...
- [x] 检测 `omp` 不存在、版本过旧、命令不可执行和路径包含空格。
- [x] 显示实际解析到的命令和版本。
...
- [ ] 支持安全重启 OMP 会话并恢复状态。
- [x] 增加诊断导出，包含脱敏后的状态、版本、命令能力和事件计数。
...
- [x] 39 个 RPC 命令的元数据、序列化和响应解包测试（`omp-rpc-protocol.test.ts`）。
- [x] 所有已知输出帧的 adapter fixture 测试（`omp-rpc-adapter.test.ts`）。
- [x] unknown frame、重复 complete、乱序 response 和进程退出测试（`omp-rpc-backend.test.ts`、`omp-rpc-diagnostics.test.ts`）。
- [x] 图片、文本、PDF、Office、音频附件测试。
- [x] 模型和思考等级映射测试。
- [x] subagent 状态 reducer 测试。
- [x] Todo、Host Tool、Host URI 单元测试（`omp-todo.test.ts`、`omp-rpc-backend.test.ts`）。

### 集成测试

- [x] 启动真实本地 OMP，完成 ready/get_state。
- [x] 普通 prompt（`omp-rpc-backend.test.ts` 本地模拟流程）。
- [x] 纯命令 prompt (`/stats`) 不挂起。
- [x] Host Tool 调用与 Host URI 读写（`omp-rpc-backend.test.ts`）。
- [x] 模型和思考等级切换。
- [x] compact/retry 运行时事件。
- [x] subagent progress/messages（后端订阅、快照恢复、transcript cursor、生命周期/进度帧和 UI 渲染测试已接入；真实 OMP 子智能体 smoke 仍待补充。）
- [x] session switch/branch/handoff/export。
- [ ] Skill 命令。
- [ ] Extension blocking UI（需真实 OMP 扩展 smoke）。
- [ ] Tool streaming update（需真实 OMP 工具 smoke）。
- [ ] 图片 prompt（需真实视觉模型 smoke）。
- [ ] OAuth 登录（需真实 OMP provider 配置 smoke）。

### Electron 端到端

验收清单与记录见 `docs/agent-batches/06-e2e-checklist.md`。
...
- [x] 真实会话 get_state。
- [x] 图片和 thinking。
- [x] 完整事件日志与未知帧诊断。（未知帧、重复/孤儿响应、写入失败、按命令 latency/timeout、command metadata、协议版本/OMP 版本探测、诊断复制导出已有；全量事件日志 UI 化持续完善。）

### Phase B：核心 OMP 产品能力（P1）

- [x] 动态斜杠命令。（基础发现、补全和热更新已接入；逐命令验证和富展示在 7.1 继续跟踪。）
- [x] 子智能体实时视图。（列表入口、详情抽屉、transcript 增量加载、错误重试、Task/Agent 工具卡跳转、协议解析与后端 reducer 测试已接入；真实 OMP smoke 仍待补充。）
- [ ] Todo。
- [ ] compact/retry/context/stats。（核心运行时、控制与统计已接入；策略编辑、错误分类、上下文安全摘要和消息分界仍待完成。）
- [x] session resume/branch/handoff/export。
- [ ] 登录和供应商状态。
