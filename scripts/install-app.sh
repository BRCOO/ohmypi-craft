...
DOWNLOAD_DIR="$HOME/.oh-my-pi/downloads"
...
if [ "$OS_TYPE" = "darwin" ]; then
    ...
    platform="linux-${arch}"
    APP_NAME="Oh-My-Pi-x86_64.AppImage"
...
if [ "$OS_TYPE" = "darwin" ]; then
    ...
    APP_BUNDLE_ID="com.ohmypi.desktop"
    if pgrep -x "Oh My Pi" >/dev/null 2>&1; then
        info "Quitting Oh My Pi..."
        ...
        if pgrep -x "Oh My Pi" >/dev/null 2>&1; then
            ...
            pkill -9 -x "Oh My Pi" 2>/dev/null || true
    ...
    echo ""
    printf "%b\n" "  Oh My Pi has been installed to ${BOLD}$INSTALL_DIR/$APP_NAME${NC}"
    echo ""
    printf "%b\n" "  You can launch it from ${BOLD}Applications${NC} or by running:"
    printf "%b\n" "    ${BOLD}open -a 'Oh My Pi'${NC}"
    echo ""
    ...
    # New paths
    APP_DIR="$HOME/.oh-my-pi/app"
    WRAPPER_PATH="$INSTALL_DIR/oh-my-pi"
    APPIMAGE_INSTALL_PATH="$APP_DIR/Oh-My-Pi-x86_64.AppImage"

    # Kill the app if it's running
    if pgrep -f "Oh-My-Pi.*AppImage" >/dev/null 2>&1; then
        info "Stopping Oh My Pi..."
        pkill -f "Oh-My-Pi.*AppImage" 2>/dev/null || true
        sleep 2
    fi
...
#!/bin/bash
# Oh My Pi launcher - handles Linux-specific AppImage issues

APPIMAGE_PATH="$HOME/.oh-my-pi/app/Oh-My-Pi-x86_64.AppImage"
ELECTRON_CACHE="$HOME/.config/@oh-my-pi"
ELECTRON_CACHE_ALT="$HOME/.cache/@oh-my-pi"

# Verify AppImage exists
if [ ! -f "$APPIMAGE_PATH" ]; then
    echo "Error: Oh My Pi not found at $APPIMAGE_PATH"
    echo "Reinstall: curl -fsSL https://raw.githubusercontent.com/BRCOO/ohmypi-craft/main/scripts/install-app.sh | bash"
    exit 1
fi
...
# AppImage creates a new /tmp/.mount_Oh-My-Pi-XXXX each launch, so any cached path is stale
for cache_dir in "$ELECTRON_CACHE" "$ELECTRON_CACHE_ALT"; do
    if [ -d "$cache_dir" ] && grep -rq '/tmp/\.mount_Oh-My-Pi' "$cache_dir" 2>/dev/null; then
        rm -rf "$cache_dir"
    fi
done
