...
echo "=== Building Oh My Pi DMG (${ARCH}) using electron-builder ==="
if [ "$UPLOAD" = true ]; then
...
cd "$ELECTRON_DIR"
bun scripts/prepare-omp-runtime.ts --platform darwin --arch "$ARCH"
...
# electron-builder.yml uses artifactName to output: Oh-My-Pi-${arch}.dmg
DMG_NAME="Oh-My-Pi-${ARCH}.dmg"
